...
public class ScheduleServiceImpl extends ServiceImpl<ScheduleMapper,Schedule> implements ScheduleService {
    ...
    public Map<String, Object> getScheduleRule(Integer page, Integer limit, String hoscode, String depcode) {
        //1 根据医院编号 和科室编号 查询MongoDB数据库
        Criteria criteria=Criteria.where("hoscode").is(hoscode).and("depcode").is(depcode);

        //2 根据工作日workDate期进行分组 综合查询并创建agg对象   注意 这里表示的是输入的第几页显示有几条数据
    ...
    public Map<String, Object> getBookingScheduleRule(Integer page, Integer limit, String hoscode, String depcode) {
        ...
        Map<Date, BookingScheduleRuleVo> scheduleVoMap = new HashMap<>();//将list对象转换为map对象
    ...
    public Schedule getScheduleId(String scheduleId) {
        ...
        Schedule schedule = this.packageSchedule(scheduleRepository.findById(scheduleId).get());
     ...
     */
       private IPage<Date> getListDate(int page, int limit, BookingRule bookingRule) {
        ...
        //计算当前预约日期
            DateTime curDateTime = new DateTime().plusDays(i);//增加天数
        ...
        int end = (page-1)*limit+limit;//最后位置
